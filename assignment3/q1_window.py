...
def make_windowed_data(data, start, end, window_size = 1):
    ...
    for sentence, labels in data:
        ...
        for i in range(sentence_len):
            window = []
            if i - window_size >= 0 and i + window_size < sentence_len:
                window = sentence[i - window_size : i + window_size + 1]
            elif i - window_size < 0 and i + window_size < sentence_len:
                window = [start] * (window_size - i) + sentence[: i + window_size + 1]
            elif i -window_size >= 0 and i + window_size >= sentence_len:
                window = sentence[i - window_size :] + [end] * (i + window_size - sentence_len + 1)
            else:
                window = [start] * (window_size - i) + sentence + [end] * (i + window_size - sentence_len + 1)

            win = [fea for word in window for fea in word]
            windowed_data.append((win, labels[i]))
		### END YOUR CODE
...
class WindowModel(NERModel):
    ...
    def add_prediction_op(self):
        ...
        W = tf.get_variable('W', [self.config.n_window_features * self.config.embed_size, self.config.hidden_size], tf.float32, initializer = tf.contrib.layers.xavier_initializer())
        b1 = tf.Variable(tf.zeros(self.config.hidden_size))
        U = tf.get_variable('U', [self.config.hidden_size, self.config.n_classes], tf.float32, initializer = tf.contrib.layers.xavier_initializer())
