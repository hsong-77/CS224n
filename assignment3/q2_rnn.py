...
def pad_sequences(data, max_length):
    ...
    for sentence, labels in data:
        ...
        pad = [0] * Config.n_features
        if len_sentence >= max_length:
            new_sentence = sentence[: max_length]
            new_label = labels[: max_length]
            mask = [True] * max_length
        else:
            new_sentence = sentence + [pad] * (max_length - len_sentence)
            new_label = labels + [4] * (max_length - len_sentence)
            mask = [True] * len_sentence + [False] * (max_length - len_sentence)
