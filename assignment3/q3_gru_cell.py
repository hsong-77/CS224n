...
class GRUCell(tf.nn.rnn_cell.RNNCell):
    ...
    def __call__(self, inputs, state, scope=None):
        ...
        with tf.variable_scope(scope):
            ...
            W_z = tf.get_variable('W_z', [self.input_size, self.state_size], initializer = tf.contrib.layers.xavier_initializer())
            U_z = tf.get_variable('U_z', [self.state_size, self.state_size], initializer = tf.contrib.layers.xavier_initializer())
            b_z = tf.get_variable('b_z', [self.state_size], initializer = tf.constant_initializer(0))
            W_r = tf.get_variable('W_r', [self.input_size, self.state_size], initializer = tf.contrib.layers.xavier_initializer())
            U_r = tf.get_variable('U_r', [self.state_size, self.state_size], initializer = tf.contrib.layers.xavier_initializer())
